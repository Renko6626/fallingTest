...
/// 施法散布骰（M4 spec §7.1）。调用点：`spell.rs::cast_all`——法术
/// `spread_bam > 0` 时，掷一次骰把出射方向在 `±spread_bam` 内偏转。
///
/// **key 用生物 id 编码进 x（`x = id as i32`、`y = 0`），不是格坐标**：生物
/// AABB 互不重叠这个假设本身不成立（M4 spec §4.2 已容许刚体/生物重叠站立），
/// 施法者所在格更谈不上"每 tick 每生物唯一"——但生物 id 本身天然唯一
/// （`Creatures` 下标即 id，从不回收），直接编码进 key 比找一个"确定唯一"
/// 的坐标代理更直接、也更省一次查询。
///
/// `salt = 槽位`：同一生物同 tick 若能连续切换槽位施法（当前规则"每生物每
/// tick 至多一发"下不会发生，但 salt 维度本身就是给"万一"的防线，不依赖
/// 调用方永远遵守这条规则），槽位不同则掷出不同值。
///
/// `attempt` **保留给未来的一 tick 多连发（multicast）**，本 Task 恒传 0——
/// spec 约束"每生物每 tick 至多一发"，attempt 这一维当前没有第二个取值可用；
/// 三个维度里唯二真正参与掷骰的是 `x`（id）与 `salt`（槽位），二者已经把
/// key 钉在 `(tick, id, slot)` 上唯一，`attempt` 现在是占位、不是缺失
/// ——总纲 §11 翻案第 4 条要求"同帧同源多次掷骰须靠 salt/attempt 区分"，
/// 本流目前只有一次掷骰源（一次施法只偏转一次方向），故不违反该纪律；
/// 一旦真的长出多连发，第 N 发填 `attempt = N` 而非新开一条流。
