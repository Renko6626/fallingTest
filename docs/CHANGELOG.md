...
- **M3 刚体 spec**（`docs/superpowers/specs/2026-09-02-m3-rigid-body-design.md`，Status: Proposed）。
  ...
  刚体密度）。**同日二次裁决改回采样式阿基米德**（更优雅、更省）：淹没体积走水面线
  采样（脚印回填计数只数到一层、力正比周长），力施于淹没质心；排开液体脱格成粒子保
  质量守恒 ⇒ 满池入箱即漫出；变换未变的刚体跳过盖章防溅射抖动。wiki 一手核查：`liquid_sand_never_box2d` 是关闭开关（沙默认托
  刚体）、`density` 只定义给液体/沙、`[box2d]` 木材全部可燃。

### Changed
- **用户签收**：M2 反应表与燃烧（spec §0 第 6 项 GIF 目检 ✅）、地图编辑器支线（端到端 ✅）。
  两份 spec Status 行与 README 优先队列同步。
- **M3 刚体改判回 M4 之前**（用户裁决：先把世界层做完整），撤回 09-01 的"延后"安排；
  `fire_oil_chain` 等既有场景不翻新。闷熄→阴燃的燃烧观感改进保留为待裁决项。
