...
## 2026-06-07 — M0.5 后（4-pass chunk 调度 + 世代戳）

| 版本 | 实测 | 备注 |
|---|---|---|
| **M0.5 后** | `128x128, 34% active, 27.1 FPS`（36.8 ms/帧） | **较 M0 后 +18%**（删每帧 O(N) 清 flag pass 的收益 > 调度循环 + 写域检查开销），基本回到 M0 前水平（27.6） |
| 多 chunk 数据点 | `192x192, 33% active, 14.0 FPS`（71.6 ms/帧） | 3×3 chunk，2.25× 像素 → 2.6× 耗时，调度开销轻微超线性、可接受 |

## 2026-06-07 — dispersion rate 后（`_probe_side` + 材质 `dispersion` 字段）

| 版本 | 实测 | 备注 |
|---|---|---|
| **dispersion 后** | `128x128, 34% active, 26.6 FPS`（37.5 ms/帧） | 较 M0.5 基线（27.2）**-2.2%**，在 10% 预算内 ✓ |
| 多 chunk 数据点 | `192x192, 33% active, 13.2 FPS`（75.6 ms/帧） | 较 M0.5 基线（13.9）**-5.0%**，预算内 ✓；`_probe_side` 探测仅在 liquid/gas 表面像素触发，额外开销极小 |
