...
总纲 §7：1280×720 上限配置，网格全图最坏 ~7ms 单线程 → 四相并行后 ~2ms；tick 总预算 ≤4ms（最坏情况）。本次全部场景（256×192 / 640×384）均小于 1280×720 上限，且本次为短程稳态口径而非全活跃最坏口径，故不直接对标 §7 数字；informal 文档已用专用 720p/1080p 场景对 §7 做过校准（8T 全活跃 ~8ms，超预算但在帧预算内），本次测量未推翻或修订该结论。M1 落地粒子层后应在 720p 量级场景上用相同短程稳态口径复测一次，观察每 cell 成本上涨幅度。（**订正 2026-08-31**：原文含"场层"，Layer F 已删除——总纲 §11 翻案记录第 6 条，复测项改为 M2 的反应表/燃烧/气体，三者均为 O(活跃格)。）
