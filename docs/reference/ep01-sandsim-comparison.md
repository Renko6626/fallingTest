...
3. **velocity 多格跳的手感方向**（`:1456-1500`）。"越落越快、能溅开"对打击感（击飞碎屑、爆炸抛射）重要。**EP01 验证了手感方向对，但其 float 实现不可联机——我们做成 8.8 定点累加器版，保手感 + 保确定。**
   ...
   > ⚠️ **运动语义差异（2026-06-14 核对 `update_sand:1456-1497`）——不止 float**：EP01 的多格移动是**"验终点瞬移"**——只算终点格 `(x+vx, y+vy)`、只检查它是否空，空就整格 teleport 过去，**不检查中途路径**。后果：velocity.y=8、中间 1~7 格有墙但第 8 格空时，**会穿墙跳到第 8 格**（终点空即可达）；终点非空时 fall-through 退回逐格只走 1 格，不会停在障碍前的最远可达处。作者自留注释 `:1410`/`:1501` *"changing the velocity here ruins everything. I need to redo this entire simulation."*——他自己也不满意这套。
   >
   > **我们必须不同**：velocity 积分要复用 `_probe_side`（`prototype/core/rules.py`）的**"沿路径逐格探测、遇障碍截停、落最远连续空格"**语义——把步数上限从 `dispersion` 换成 `floor(|velocity|)` 即可。这避免穿墙，比 EP01 的"验终点瞬移"正确。dispersion 已铺好这条路径探测基础设施。
