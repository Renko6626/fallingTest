...
- **淹没体积 = 接触门控的水面线采样**（方案 1，2026-09-03 决策记录第 14 条；不是"数流进脚印的液体格"——一 tick 只流进一层，力正比于周长）。**选列**：遍历脚印像素，左右邻格是非本体 Liquid 即接触；每个接触列沿接触行向外穿过连续液体，取边外第 2 格起最多 `SURFACE_REACH = 5` 格的列为采样列（紧邻列只在没有远列时兜底）——连通性由接触行保证，隔壁的水槽走不过来。**读数**：每列在 AABB 行带内自上而下找第一个 Liquid 格（对水里的气泡免疫）；先碰到刚体格的列作废；各列取**最低者** `h`（溅到高处的水只抬高单列读数）；全部作废退到接触格最高者。一个接触都没有 ⇒ 未淹没。淹没量 = 占位像素按**连续世界中心**与 `y ≥ h` 半平面的竖向重叠分数 `w ∈ [0,1]` 之和 `n_sub = Σw`（连续于位姿，浮体才有真平衡点、能入睡），`ρ_liq` 取接触格里出现最多的液体材质的 `density`。已知限制：两侧都贴墙（卡在同宽槽里）⇒ 无接触 ⇒ 无浮力。
- **力**：`F_浮 = n_sub × ρ_liq × g` 逆重力，施于**淹没像素质心**（白送扶正力矩，箱子会自己翻正）；阻力 `F_阻 = −K_DRAG × n_sub × v_质心`，角阻力 `τ_阻 = −K_DRAG × Σ w|r|² × ω`（同一系数：即"每个淹没像素受 −K·w·v_i"的合力与合力矩；2026-09-03 补，没有它横摇永不衰减）。引擎重力照常施加 `−m g`，净力即 `(n_sub ρ_liq − n_total ρ_body) g`——真阿基米德，与尺寸无关。
...
- **顶面载荷、沉降闸门、密封支撑**（2026-09-03，决策记录第 16 条）：① 堆在顶面像素上、**高于周围自由面**（判据：该行在所有采样列里都不是沉降液体）的液体按格 × `ρ_liq·g` 往下压，施于其加权中心；睡眠刚体顶上堆到 `WAKE_TOP_LOAD_CELLS = 16` 格即唤醒。不能用"`y < h`"判水丘——`h` 只在 AABB 行带内扫，全淹没箱子的整根池水柱会被当成水丘（实测压到池底）。② 只有竖直速度 `vel < SETTLED_VEL_MAX = 2 格/tick` 的液体格算接触/载荷：落水流擦过箱子不给浮力；阈值不能取 0，入水冲击扰动的池水 ≈ 1 格/tick，取 0 会让木条入水不减速砸到池底。③ 没有侧面液体接触、底面贴着沉降液体、且所有侧面邻格都是硬格 ⇒ 密封塞子压着封闭水柱：向下速度截断 + 抵消重力 + 阻力（不可压缩，撞上即停；只加阻力会以 268 格/s 犁到槽底）。
...
- `Op::SpawnBody { material, x, y, w, h, angle_deg }`（RON `SpawnBody(material: "wood", x: 100, y: 40, w: 24, h: 16, angle_deg: 35)`，`angle_deg` 整数度、缺省 0，2026-09-03 加）：加载期校验材质 Static、尺寸 ≥ 阈值、落在世界内；生成 = 全占位图 → §4 形状 → 插入引擎（静止无旋转）→ 下一 tick 首次盖章。`MAX_BODIES = 256` 超限确定性拒绝并计数（粒子池先例）。
...
12. **2026-09-03 追加：`SpawnBody.angle_deg`**（整数度，`physics` 边界一次转 f32 弧度）——用于
    "长条斜着入水被浮力扶正"的展示与执法测试 `tilted_plank_rights_itself_in_water`
    （32×6 木条 35° 入水，稳定后 bbox 高 ≤ 9 行、漂在水面附近）；`crate_yard` 同步加入。
13. **2026-09-03 目检修订："水里的刚体越转越快"**——深水塘里 32×6 木条入水后 |ω| 单调爬到
    22 rad/s 整圈翻滚。探针实证的链条：运动中的刚体把排开的水以**自身线速度**弹出，粒子立刻
    落回迎水面、随刚体一起被抬着走，尾流留下空腔 ⇒ 刚体贴着一座自己推出来的水丘出水；
    `surface_line` 的 4 列恰好落在水丘里，`h` 比真实水面高 18 格 ⇒ 判"192/192 全淹没" ⇒
    以 g/3 猛推出水（阻力限速 18 格/s）⇒ 横拍回水面时单端入水、力臂 9 格、力矩与 ω 同号
    做正功 ⇒ 假势能全转成自旋；而阻力只阻线速度、角速度零耗散，能量只能留在旋转里。
    三处改（§5）：① `h` 取各列**最低者**并加远列（自溅只抬高读数，偏低是自限的）；② 淹没量按
    像素连续中心分数计权——第 11 条 ③ 声称的"像素分数"当时**并未落地**（`a04ebd0` 只改了
    不唤醒与阈值，`body.rs` 仍是整行 `y ≥ h` 计数），整行台阶让 32 宽木条在 ±1.7 格/s 间抖、
    永远过不了睡眠阈值，此次真正落地；③ 角阻力 `−K·Σw|r|²·ω`。结果：木条 |ω| 峰值 1.6、
    tick 500 入睡；三体同池 tick 2000 前全部入睡；`crate_yard` 5000 tick 1.07 → 0.72 ms/tick。
    行为测试 `plank_in_deep_pool_does_not_spin_up`；`Sim::body_state` 只读诊断视图。
14. **2026-09-03 方案 1：接触门控水面线**（用户裁决，同日候选方案对照：0 远场采样 / 1 接触
    门控 / 2 Noita 逐像素反作用 / 3 FSS v2 不盖章回调 / 4 wet 位记忆 / 5 湿面静水压）。方案 0 的
    结构性错误：采样列与刚体之间没有"接触"约束，隔着玻璃壁的水槽、架高水槽里的水都被当成
    水面，地上的箱子悬浮到水槽水面高度并入睡（实测两例）；第 13 条加的远列把触达从 2 格扩到
    5 格，更容易触发。改为：列由脚印像素的左右邻格接触决定、沿接触行向外连通、读数仍自上而下
    取最低（§5）。实施期两条实测教训：① 从接触向上扫连续液体会塌到瞬时气泡处，木箱/木条
    前 1500 tick 每 tick 重盖章、弹水 5000 粒、睡不着——读数必须自上而下；② 斜放刚体的下斜面
    邻格就在本体正下方，向上一格即被挡住，若当水面则升力归零（35° 木条不减速砸到池底）——
    被刚体挡住的列作废。方案 5 的已知缺陷记录：底面深度局部不可测（升力仍要借接触线）、列高
    ≠ 压强（30 格水帘沿侧面流下 ≈ 2 倍自重侧推）、横向/力矩通道让 CA 噪声注能、密封底面。
    后续若目检嫌水丘假，下一步是给方案 1 加"顶面载荷"项（只数压在顶面像素上的连续液体列），
...
16. **2026-09-03 浮力尾巴收口**（§5 三条 + 撤回一条）：顶面载荷、沉降闸门、密封支撑落地，
    行为测试 `heap_on_floating_crate_pushes_it_down` / `water_stream_beside_crate_gives_no_lift` /
    `plug_in_sealed_channel_rests_on_trapped_water` / `plank_resting_on_floating_crate_is_not_lifted`，
    单测顶面载荷计数、沉降闸门阈值、密封判定。**撤回**：排开粒子按格点速度 `v + ω×r` 出射——
    实测横摇中的木条把水甩得不对称 ⇒ 接触列 `h` 跟着抖 ⇒ 维持横摇，3000 tick 一次都睡不着
    （四项开关二分定位），出射速度仍只带质心线速度。`crate_yard` 木条改为 tick 700 在池右半
