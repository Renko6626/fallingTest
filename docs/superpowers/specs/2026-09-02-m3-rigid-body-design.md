...
- **淹没体积 = 水面线采样**（不是"数流进脚印的液体格"——一 tick 只流进一层，那会把力算成正比于周长）：对每个清醒刚体，采 AABB 两侧各紧邻 2 列 + 各隔 3 格的远 2 列（共 8 列，都在脚印之外），自上而下找第一个 Liquid 格得各列水面；先碰到别的刚体格的列作废；取**最低者** `h`（y 最大，整数；2026-09-03 由中位数改，决策记录第 13 条：刚体自己推起的水丘只会抬高读数，取最低才不被自己的溅水俘获）；一列都没有 ⇒ 未淹没。淹没量 = 刚体占位像素按**连续世界中心**（局部像素中心经引擎变换）与 `y ≥ h` 半平面的竖向重叠分数 `w ∈ [0,1]` 之和 `n_sub = Σw`（连续于位姿，浮体才有真平衡点、能入睡），`ρ_liq` 取各列水面格里出现最多的液体材质的 `density`。
