...
设全扫访问序 V = 行自下而上、行内按**每 tick 掷一次的全局奇偶相位** `(y ^ flip) & 1` 定向（`flip = rng::scan_flip(fseed)`；2026-08-31 由 `(y+tick)` 奇偶订正而来，见 `docs/proposals/2026-08-31-powder-scan-direction-bias.md`），每 cell 恰访问一次、不回访。

> **本节论证对行方向取哪个值不敏感**——方向只决定"未访问侧"是左还是右，承载等价性的是下文"起点用快照、终点每步重读"的非对称结构。但它**要求方向在三种 ScanMode 下取值一致**，故有红线：**行方向必须是 `(tick, y)` 的纯函数，禁读活矩形/扫描起始矩形/`own_ci`/脏状态/线程上下文**（charter §11 实施期决策第 3 条）。违反即三模式分叉。
