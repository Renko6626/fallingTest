...
**Task 2 实测 vs 预期**：golden 预期"四个全变（含纯沙 `sand_pile`，反向哨兵）、两条 `_fp` 行不变"**完全兑现**。一条**预期之外**的观察：`waterfall_ci` 只有 tick 256 一行变，512/768/1024/final 逐位收敛回旧值——稳态下所有 cell 速度位恒 0，这是"撞停清零 + 静止零写入"在哈希层的独立佐证（不是设计时想到的判据，事后看比任何单测都硬）。

**Task 3 开工前须知**：§6.1①（`MovedSide` 是否触发溅射）仍未决，Task 3 目检后终裁。§4.2④（斜滑是否清零速度）已于 2026-08-31 终裁——取默认"不清零"，见 §8 第 9 条。
...
**为什么无符号**：粉末与液体只向下运动；向上运动的气体走专用规则分支（不占本位段），撞击反弹走 Layer P（脱格成粒子）。省一位。
> **订正（2026-08-31）**：本句原作"向上运动的气体走 Layer F"。Layer F 场层已整体删除（`docs/overview/kernel-charter.md` §11 翻案记录第 6 条），气体改为 Layer G 的第四个 Category、走专用规则分支。**无符号的结论与位段分配不变**，仅理由换锚。
