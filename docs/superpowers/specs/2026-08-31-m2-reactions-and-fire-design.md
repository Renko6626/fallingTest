...
1. ✅ **总纲 M2 标准**：`fire_oil_chain` golden 全哈希流回归；SyncTest 六配置 2 万 tick 零分叉（199.6s）。
2. ✅ **线程数不变性**：六配置矩阵含 1/4 线程 × 三 ScanMode；bench 侧 1/8/16 线程另证。
...
4. **Gas 加 `rise_chance` 字段（×255 量化，缺省 1.0 = 恒上浮 = §3.2 原语义），
   fire 取 0.5**（2026-08-31 二次目检修订）。根因：burn/点燃在运动后的落点执行，
   恒升的火升离水平燃料面后，落点下邻是**自己刚腾出的空气**——4 邻里永远没有
   刚离开的表面（实测 13 格火 × 40 tick 寿命 0 次点燃；埋在液体里反而可点是
   因为 displace 交换把燃料换进尾流）。Noita 查证：flame 寿命仅 0.083s（≈5 帧）
   且**贴着燃料闪烁而非直升**（wiki + `noita-deep-dive.md:46`），火焰由燃烧格
   的 `generates_flames` 持续再生。逗留 tick 零写入，smoke 等缺省恒升、行为
   逐位不变；`STREAM_GASSTAY = 8`。回归测试
   `fire_dropped_on_surface_ignites_pool`（修复前是不可能事件）。§3.2 的
   "恒定 1 格/tick"自本条起按 rise_chance 概率化。
