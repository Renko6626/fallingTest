...
| Task | 内容 | 状态 |
|---|---|---|
| 0 | 数据层：材质新字段 + `reactions.ron` + 加载期契约 | ⬜ |
| 1 | 气体：`Category::Gas` + `gas_step` | ⬜ |
...
`Category::Gas` 走 `gas_step`，与 `powder_step` 严格镜像：**向上 → 两个斜上 → 水平扩散**。
...
反应命中后：两格都写产物 + **盖当前戳**（戳同时防住帧内连锁沿扫描方向蔓延）；**产物的 `counter` 重置为产物材质的初值**（`lifetime` 或 0）；**速度清零**——材质换了，动量语义不再成立。
...
```
源.fire_temp ≥ 目标.ignition_temp  且  目标.fire_hp > 0  且  目标.counter == 0
```
