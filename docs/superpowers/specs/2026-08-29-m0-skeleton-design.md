...
> **实施期修订（2026-08-30）**：初版"cell 级冻结矩形扫描"被 SyncTest 当场击落——
> 单缓冲扫描允许**tick 内链式移动**（整段静止的水在一个 tick 内沿扫描方向整体平移，
> 链长无上界），tick 起点冻结的矩形会把链切断，与全扫语义分叉（实测 tick 583，
> 一段旧戳水行只动了右端）。修复：**休眠粒度提升为 chunk 级 + 相位边界唤醒**，
> 活跃 chunk 全量扫描。cell 级矩形扫描若未来重新引入，必须连同"链式移动禁令"
> 一起作为语义变更走决策日志。

- 每 chunk 双矩形：`dirty`（上 tick 标记快照）、`next_dirty`（本 tick 写入积累，tick 末交换清空）。
- 任何 cell 写 (x,y) ⇒ 在**所辖 chunk** 的 `next_dirty` 并入 (x,y)±1 邻域；邻域越过 chunk 边界的部分
  并入邻 chunk（唤醒）。M0 单步移动 ≤1 格，±1 保守充分；引入更远移动时此常数随 r 审计。
- **调度判定（每个相位边界重查）**：chunk 被扫描 ⟺ `dirty` 非空 ∨ `next_dirty` 已有标记
  （本 tick 更早相位的写入）。相位屏障后原子合并结果与线程调度无关 ⇒ 判定确定。
  等价论证：睡眠 chunk 的 cell 上 tick 末不可动，且本 tick 至今无 ±1 邻域写入 ⇒ 仍不可动
  （可动性只依赖 5 邻域）；chunk 内链式移动因全量扫描与全扫语义一致；跨 chunk 链在
  相位边界被唤醒判定接住。**该等价由 SyncTest「跳过开 vs 关」比对持续执法**（§5）。
...
- 矩形数据本身保留（未来渲染增量上传 / 优化用），但 M0 不再决定扫描细粒度。
