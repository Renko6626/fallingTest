...
> **订正（2026-08-31）**：②号差距**已消失**——总纲 §11 翻案记录第 6 条删除了 Layer F 场层，
> 温度与气体回归 Noita 体系（温度 = 材质静态常量 + 反应表、气体 = Layer G 第四类）。
> **O2 随前提消失而作废**（详见下方 O2 小节）。本文其余三项不受影响。

| # | 项 | 时点 | 预期收益 |
|---|---|---|---|
| O1 | chunk 内活矩形扫描 | **M1 门口**（粒子层动工前） | 活跃 chunk 内静区扫描归零；稀疏活跃场景数倍 |
| ~~O2~~ | ~~Layer F 低分辨率场格 + 半频更新~~ | **作废（2026-08-31）** | 前提（Layer F）已被总纲翻案 6 删除 |
...
## ~~O2 Layer F 降本（低分辨率场格 + 半频）~~ —— 作废（2026-08-31）

> **作废理由**：本项的前提是 Layer F 场层存在。总纲 §11 翻案记录第 6 条（2026-08-31，用户裁决）
> **删除了整个场层**——温度改为材质静态常量 + 反应表、气体改为 Layer G 的第四个 Category，
> 都是 O(活跃格) 的局部计算，不再有需要降本的全图 pass。本节原文保留，作为**复议时的现成方案**：
> 若 M4 法术设计确认需要连续温度量而重开场层，本节的"4×4 场格 + 半频"是重开时的首选形态
> （翻案 6 明确要求重开前先用它做 bench，而非直接上全分辨率全频）。
