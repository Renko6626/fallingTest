...
| 文档 | 管什么 |
|---|---|
| `docs/overview/kernel-charter.md` | **宪法**：第一性原则 P1–P5、三层内核（四相 push 网格 + 稀疏粒子 + pull 场）、联机模型、确定性法典、里程碑 M0–M6、决策日志与翻案记录 |
...
M0 骨架与执法（chunk 存储 + 四相调度 + 沙/水 + SyncTest 框架；验收：双机 10 万 tick 零分叉）→ M1 粒子层 → M2 场层与反应表 → M3 刚体 → M4 玩家与法术 → M5 联机对局 → M6 rollback 决策门。验收标准全文见总纲 §11。
